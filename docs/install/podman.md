---
summary: "Run OrionClaw in a rootless Podman container"
read_when:
...
Run the OrionClaw gateway in a **rootless** Podman container. Uses the same image as Docker (build from the repo [Dockerfile](https://github.com/aaronboshart1/orionclaw/blob/main/Dockerfile)).
...
This also creates a minimal `~openclaw/.openclaw/orionclaw.json` (sets `gateway.mode="local"`) so the gateway can start without running the wizard.
...
- **Home:** e.g. `/home/openclaw` — holds `~/.orionclaw` (config, workspace) and the launch script `run-openclaw-podman.sh`.
...
- **Config:** Only `openclaw` and root can access `/home/openclaw/.openclaw`. To edit config: use the Control UI once the gateway is running, or `sudo -u openclaw $EDITOR /home/openclaw/.openclaw/orionclaw.json`.
...
- **Optional:** In that `.env` you can set provider keys (e.g. `GROQ_API_KEY`, `OLLAMA_API_KEY`) and other OrionClaw env vars.
...
- **Gateway bind:** By default, `run-openclaw-podman.sh` starts the gateway with `--bind loopback` for safe local access. To expose on LAN, set `OPENCLAW_GATEWAY_BIND=lan` and configure `gateway.controlUi.allowedOrigins` (or explicitly enable host-header fallback) in `orionclaw.json`.
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~openclaw/.openclaw/orionclaw.json` exists and sets `gateway.mode="local"`. `setup-podman.sh` creates this file if missing.
