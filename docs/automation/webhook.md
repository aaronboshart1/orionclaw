...
read_when:
  ...
  - Wiring external systems into OrionClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your OrionClaw config directory (typically `~/.orionclaw/hooks/transforms`).
...
- `orionclaw webhooks gmail setup` writes `hooks.gmail` config for `orionclaw webhooks gmail run`.
