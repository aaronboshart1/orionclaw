...
OrionClaw exposes OAuth expiry health via `orionclaw models status`. Use that for
...
- `scripts/claude-auth-status.sh` now uses `orionclaw models status --json` as the
...
- `scripts/claude-auth-status.sh`: Claude Code + OrionClaw auth checker (full/json/simple).
...
- `scripts/termux-sync-widget.sh`: sync Claude Code creds → OrionClaw.
