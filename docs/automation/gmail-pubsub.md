---
summary: "Gmail Pub/Sub push wired into OrionClaw webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to OrionClaw
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> OrionClaw

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> OrionClaw webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- OrionClaw hooks enabled (see [Webhooks](/automation/webhook)).
...
under `~/.orionclaw/hooks/transforms` (see [Webhooks](/automation/webhook)).

## Wizard (recommended)

Use the OrionClaw helper to wire everything together (installs deps on macOS via brew):
...
- Writes `hooks.gmail` config for `orionclaw webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, OrionClaw automatically sets
...
- `--hook-url` points to OrionClaw `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to OrionClaw.

Recommended: `orionclaw webhooks gmail run` wraps the same flow and auto-renews the watch.
