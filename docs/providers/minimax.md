---
summary: "Use MiniMax M2.1 in OrionClaw"
read_when:
  - You want MiniMax models in OrionClaw
...
orionclaw gateway restart  # restart if gateway is already running
orionclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/aaronboshart1/orionclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `orionclaw.json`:
...
- Use `orionclaw models list` and `orionclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `orionclaw configure` and selecting **MiniMax M2.1**, or
