---
summary: "Use OpenAI via API keys or Codex subscription in OrionClaw"
read_when:
  - You want to use OpenAI models in OrionClaw
...
```bash
orionclaw onboard --auth-choice openai-api-key
# or non-interactive
orionclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
orionclaw onboard --auth-choice openai-codex
...
OrionClaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), OrionClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. OrionClaw enables it by default for
...
`baseUrl` on `api.openai.com`), OrionClaw now auto-enables OpenAI server-side
