...
This plan defines how OrionClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- OrionClaw core owns ACP control plane concerns
...
OrionClaw should not reimplement ACP transport internals in core.
OrionClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in OrionClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under OrionClaw state dir
...
Core (must be in OrionClaw core):
...
- ACP event stream is normalized to OrionClaw reply chunks
...
For the first production backend (`extensions/acpx`), OrionClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses OrionClaw `sessionKey`)
...
- OrionClaw consumes ndjson events from `acpx --format json --json-strict`
