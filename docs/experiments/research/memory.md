...
read_when:
  - Designing workspace memory (~/.orionclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OrionClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.orionclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.orionclaw/workspace` as canonical human-readable memory.
...
```
~/.orionclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OrionClaw**, but keep a separable core library.

### Why integrate into OrionClaw?

- OrionClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `orionclaw memory recall "…" --k 25 --since 30d`
  - `orionclaw memory reflect --since 7d`
...
Pragmatic take for `~/.orionclaw/workspace`:
