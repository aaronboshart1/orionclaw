...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OrionClaw
...
Running OrionClaw via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `orionclaw status` and `gateway:watch` worked.
