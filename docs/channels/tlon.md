...
Tlon is a decentralized messenger built on Urbit. OrionClaw connects to your Urbit ship and can
...
```bash
openclaw plugins install @orionclaw/tlon
```
...
By default, OrionClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `orionclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OrionClaw replies in-thread.
