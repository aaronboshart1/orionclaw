...
read_when:
  - Setting up Twitch chat integration for OrionClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OrionClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openclaw plugins install @orionclaw/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "orionclaw", // Bot's Twitch account
