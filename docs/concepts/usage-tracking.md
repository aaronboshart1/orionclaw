...
- `/usage cost` in chats: local cost summary aggregated from OrionClaw session logs.
- CLI: `orionclaw status --usage` prints a full per-provider breakdown.
- CLI: `orionclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
