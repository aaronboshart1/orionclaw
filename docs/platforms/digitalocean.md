---
summary: "OrionClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OrionClaw on DigitalOcean
  - Looking for cheap VPS hosting for OrionClaw
title: "DigitalOcean"
---

# OrionClaw on DigitalOcean

## Goal

Run a persistent OrionClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status orionclaw-gateway.service

# View logs
journalctl --user -u orionclaw-gateway.service -f
```
...
```bash
orionclaw channels login whatsapp
# Scan QR code
```
...
- `~/.orionclaw/workspace/` — workspace (SOUL.md, memory, etc.)
