...
- Set `gateway.bind: "tailnet"` in `~/.orionclaw/orionclaw.json` on the gateway host.
...
1. Create `~/.orionclaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
orionclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__openclaw__/canvas/"}'
```
