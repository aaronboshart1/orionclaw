...
LINE 通过 LINE Messaging API 连接到 OrionClaw。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
openclaw plugins install @orionclaw/line
```
