...
summary: 完全卸载 OrionClaw（CLI、服务、状态、工作区）
title: 卸载
x-i18n:
...
- 如果你使用了配置文件（`--profile` / `OPENCLAW_PROFILE`），对每个状态目录重复步骤 3（默认为 `~/.orionclaw-<profile>`）。
...
默认单元名称是 `orionclaw-gateway.service`（或 `orionclaw-gateway-<profile>.service`）：
...
默认任务名称是 `OrionClaw Gateway`（或 `OrionClaw Gateway (<profile>)`）。
任务脚本位于你的状态目录下。
...
如果你使用了 `https://orionclaw.local/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g openclaw@latest` 安装的。
使用 `npm rm -g openclaw` 移除（或 `pnpm remove -g` / `bun remove -g`，如果你是用那种方式安装的）。

### 源码检出（git clone）

如果你从仓库检出运行（`git clone` + `orionclaw ...` / `bun run openclaw ...`）：
