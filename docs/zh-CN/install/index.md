...
read_when:
  - 安装 OrionClaw
  - 你想从 GitHub 安装
summary: 安装 OrionClaw（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
- 运行新手引导：`orionclaw onboard --install-daemon`
- 快速检查：`orionclaw doctor`
- 检查 Gateway 网关健康状态：`orionclaw status` + `orionclaw health`
- 打开仪表板：`orionclaw dashboard`
...
```bash
# 显式 npm
curl -fsSL https://orionclaw.local/install.sh | bash -s -- --install-method npm
