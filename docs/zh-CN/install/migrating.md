---
read_when:
  - 你正在将 OrionClaw 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 OrionClaw 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南将 OrionClaw Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$OPENCLAW_STATE_DIR`，默认：`~/.orionclaw/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.orionclaw/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.orionclaw-<profile>/`）
...
如果你有多个配置文件/状态目录（例如 `~/.orionclaw-main`、`~/.orionclaw-work`），分别归档每个。

### 步骤 1 — 在新机器上安装 OrionClaw

在**新**机器上，安装 CLI（如果需要还有 Node）：

- 参见：[安装](/install)

在这个阶段，如果新手引导创建了一个新的 `~/.orionclaw/` 也没关系 — 你将在下一步覆盖它。

### 步骤 2 — 将状态目录 + 工作区复制到新机器

复制**两者**：

- `$OPENCLAW_STATE_DIR`（默认 `~/.orionclaw/`）
- 你的工作区（默认 `~/.orionclaw/workspace/`）
...
### 陷阱：只复制 `orionclaw.json`

`orionclaw.json` 是不够的。许多提供商在以下位置存储状态：
...
- `orionclaw status` 显示 Gateway 网关正在运行
...
- [OrionClaw 在哪里存储数据？](/help/faq#where-does-openclaw-store-its-data)
