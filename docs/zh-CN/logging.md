...
你可以在 `~/.orionclaw/orionclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.orionclaw/orionclaw.json` 的 `logging` 下。
...
- OrionClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `orionclaw plugins enable diagnostics-otel` 启用插件。
...
- **Gateway 网关无法访问？** 先运行 `orionclaw doctor`。
