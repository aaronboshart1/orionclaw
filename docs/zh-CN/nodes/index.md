...
macOS 也可以在**节点模式**下运行：菜单栏应用连接到 Gateway 网关的 WS 服务器，并将其本地 canvas/camera 命令作为节点暴露（因此 `orionclaw nodes …` 可以针对这台 Mac 工作）。
...
- `node.pair.*`（CLI：`orionclaw nodes pending/approve/reject`）是一个单独的 Gateway 网关拥有的
...
- **批准**：通过 `~/.orionclaw/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.orionclaw/orionclaw.json`）。
- `orionclaw node run` 读取 `OPENCLAW_GATEWAY_TOKEN` 进行认证。
...
orionclaw nodes list
```

命名选项：

- 在 `orionclaw node run` / `orionclaw node install` 上使用 `--display-name`（持久化在节点上的 `~/.orionclaw/node.json` 中）。
- `orionclaw nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。
...
批准存储在节点主机的 `~/.orionclaw/exec-approvals.json` 中。
...
orionclaw nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.orionclaw/exec-approvals.json`）。
...
OrionClaw 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.orionclaw/node.json` 中。
- Exec 批准通过 `~/.orionclaw/exec-approvals.json` 在本地执行
...
- macOS 菜单栏应用作为节点连接到 Gateway 网关 WS 服务器（因此 `orionclaw nodes …` 可以针对这台 Mac 工作）。
