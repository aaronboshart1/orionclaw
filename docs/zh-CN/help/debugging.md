...
当你需要切换不常用的设置而不编辑 `orionclaw.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.orionclaw-dev` 下，并将 Gateway 网关端口默认为 `19001`（派生端口随之移动）。
...
OrionClaw 可以在任何过滤/格式化之前记录**原始助手流**。
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.orionclaw/logs/raw-stream.jsonl
```
