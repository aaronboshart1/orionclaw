...
OrionClaw 通过 `orionclaw models status` 提供 OAuth 过期健康状态。请使用该命令进行自动化和告警；脚本是为手机工作流程提供的可选附加功能。
...
- `scripts/claude-auth-status.sh` 现在使用 `orionclaw models status --json` 作为数据来源（如果 CLI 不可用则回退到直接读取文件），因此请确保 `openclaw` 在定时器的 `PATH` 中。
...
- `scripts/claude-auth-status.sh`：Claude Code + OrionClaw 认证检查器（完整/json/简洁模式）。
...
- `scripts/termux-sync-widget.sh`：同步 Claude Code 凭证 → OrionClaw。
