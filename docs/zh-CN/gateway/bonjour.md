...
OrionClaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
OrionClaw 支持任何发现域名；`openclaw.internal.` 只是一个示例。
...
- 从 `~/.orionclaw/dns/<domain>.db` 提供你选择的域名服务（示例：`openclaw.internal.`）
...
- 在 `~/.orionclaw/orionclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
- `~/.orionclaw/orionclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
