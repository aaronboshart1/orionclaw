...
summary: 设置指南：在保持最新的同时保持你的 OrionClaw 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.orionclaw/workspace`（工作区）+ `~/.orionclaw/orionclaw.json`（配置）。
...
- **配置：** `~/.orionclaw/orionclaw.json`（JSON/JSON5 格式）
- **工作区：** `~/.orionclaw/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
- 运行 `orionclaw setup`，然后 `orionclaw channels login`，然后手动启动 Gateway 网关（`orionclaw gateway`）。
...
- **配对允许列表**：`~/.orionclaw/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.orionclaw/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.orionclaw/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.orionclaw/workspace` 和 `~/.orionclaw/` 保持为"你的东西"；不要将个人提示/配置放入 `openclaw` 仓库。
