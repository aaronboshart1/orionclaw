...
read_when:
  - 设计 macOS 新手引导助手
  - 实现认证或身份设置
summary: OrionClaw 的首次运行新手引导流程（macOS 应用）
title: 新手引导
x-i18n:
...
- 打开浏览器进行 OAuth（PKCE）
- 要求用户粘贴 `code#state` 值
- 将凭证写入 `~/.orionclaw/credentials/oauth.json`
...
在首次智能体运行时，OrionClaw 会引导一个工作区（默认 `~/.orionclaw/workspace`）：
...
- `~/.orionclaw/credentials/oauth.json`
