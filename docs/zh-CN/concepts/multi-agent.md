...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.orionclaw/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.orionclaw/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置：`~/.orionclaw/orionclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
- 状态目录：`~/.orionclaw`（或 `OPENCLAW_STATE_DIR`）
- 工作区：`~/.orionclaw/workspace`（或 `~/.orionclaw/workspace-<agentId>`）
- 智能体目录：`~/.orionclaw/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.orionclaw/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，OrionClaw 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.orionclaw/workspace`（或当设置了 `OPENCLAW_PROFILE` 时为 `~/.orionclaw/workspace-<profile>`）。
- 状态默认为 `~/.orionclaw/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.orionclaw/credentials/whatsapp/personal
