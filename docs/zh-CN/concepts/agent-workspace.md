...
这与 `~/.orionclaw/` 是分开的，后者存储配置、凭证和会话。
...
当启用沙箱隔离且 `workspaceAccess` 不是 `"rw"` 时，工具在 `~/.orionclaw/sandboxes` 下的沙箱工作区内操作，而不是你的主机工作区。
...
- 在 `~/.orionclaw/orionclaw.json` 中覆盖：
...
`orionclaw onboard`、`orionclaw configure` 或 `orionclaw setup` 将创建工作区并在缺失时填充引导文件。
...
`orionclaw doctor` 在检测到额外工作区目录时会发出警告。

## 工作区文件映射（每个文件的含义）

这些是 OrionClaw 在工作区内期望的标准文件：
...
如果任何引导文件缺失，OrionClaw 会在会话中注入"缺失文件"标记并继续。大型引导文件在注入时会被截断；使用 `agents.defaults.bootstrapMaxChars` 调整限制（默认：20000）。
`orionclaw setup` 可以重新创建缺失的默认值而不覆盖现有文件。

## 工作区中不包含的内容

这些位于 `~/.orionclaw/` 下，不应提交到工作区仓库：

- `~/.orionclaw/orionclaw.json`（配置）
- `~/.orionclaw/credentials/`（OAuth token、API 密钥）
- `~/.orionclaw/agents/<agentId>/sessions/`（会话记录 + 元数据）
- `~/.orionclaw/skills/`（托管的 Skills）
...
如果你必须存储敏感引用，请使用占位符并将真正的密钥保存在其他地方（密码管理器、环境变量或 `~/.orionclaw/`）。
...
1. 将仓库克隆到所需路径（默认 `~/.orionclaw/workspace`）。
2. 在 `~/.orionclaw/orionclaw.json` 中将 `agents.defaults.workspace` 设置为该路径。
3. 运行 `orionclaw setup --workspace <path>` 来填充任何缺失的文件。
4. 如果你需要会话，请单独从旧机器复制 `~/.orionclaw/agents/<agentId>/sessions/`。
