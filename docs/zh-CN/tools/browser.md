...
OrionClaw 可以运行一个由智能体控制的**专用 Chrome/Brave/Edge/Chromium 配置文件**。
...
- `chrome`：到你**系统浏览器**的扩展中继（需要将 OrionClaw 扩展附加到标签页）。

如果你希望默认使用托管模式，请设置 `browser.defaultProfile: "orionclaw"`。

## 配置

浏览器设置位于 `~/.orionclaw/orionclaw.json`。
...
- 默认配置文件是 `chrome`（扩展中继）。使用 `defaultProfile: "orionclaw"` 来使用托管浏览器。
...
如果你的**系统默认**浏览器是基于 Chromium 的（Chrome/Brave/Edge 等），OrionClaw 会自动使用它。设置 `browser.executablePath` 可覆盖自动检测：
...
- **远程 CDP：** 设置 `browser.profiles.<name>.cdpUrl`（或 `browser.cdpUrl`）以附加到远程的基于 Chromium 的浏览器。在这种情况下，OrionClaw 不会启动本地浏览器。
...
OrionClaw 在调用 `/json/*` 端点和连接 CDP WebSocket 时会保留认证信息。建议使用环境变量或密钥管理器存储令牌，而不是将其提交到配置文件中。

## 节点浏览器代理（零配置默认）

如果你在有浏览器的机器上运行**节点主机**，OrionClaw 可以自动将浏览器工具调用路由到该节点，无需任何额外的浏览器配置。这是远程 Gateway 网关的默认路径。
...
[Browserless](https://browserless.io) 是一个托管的 Chromium 服务，通过 HTTPS 暴露 CDP 端点。你可以将 OrionClaw 浏览器配置文件指向 Browserless 区域端点，并使用你的 API 密钥进行认证。
...
OrionClaw 支持多个命名配置文件（路由配置）。配置文件可以是：
...
OrionClaw 还可以通过本地 CDP 中继 + Chrome 扩展驱动**你现有的 Chrome 标签页**（无需单独的"orionclaw"Chrome 实例）。
...
- 你点击标签页上的 **OrionClaw Browser Relay** 扩展图标来附加（它不会自动附加）。
...
- "加载已解压的扩展程序" → 选择 `orionclaw browser extension path` 打印的目录
...
如果你看到 `Playwright is not available in this gateway build`，请安装完整的 Playwright 包（不是 `playwright-core`）并重启 Gateway 网关，或者重新安装带浏览器支持的 OrionClaw。
...
- `orionclaw browser download e12 /tmp/report.pdf`
- `orionclaw browser waitfordownload /tmp/report.pdf`
- `orionclaw browser upload /tmp/file.pdf`
...
- `orionclaw browser set headers --json '{"X-Debug":"1"}'`
...
OrionClaw 支持两种"快照"风格：

- **AI 快照（数字 ref）**：`orionclaw browser snapshot`（默认；`--format ai`）
  - 输出：包含数字 ref 的文本快照。
  - 操作：`orionclaw browser click 12`、`orionclaw browser type 23 "hello"`。
  - 内部通过 Playwright 的 `aria-ref` 解析 ref。

- **角色快照（角色 ref 如 `e12`）**：`orionclaw browser snapshot --interactive`（或 `--compact`、`--depth`、`--selector`、`--frame`）
  - 输出：带有 `[ref=e12]`（和可选的 `[nth=1]`）的基于角色的列表/树。
  - 操作：`orionclaw browser click e12`、`orionclaw browser highlight e12`。
...
3. 如果仍然失败：`orionclaw browser highlight <ref>` 查看 Playwright 定位的目标
4. 如果页面行为异常：
   - `orionclaw browser errors --clear`
   - `orionclaw browser requests --filter api --clear`
5. 深度调试：录制 trace：
   - `orionclaw browser trace start`
   - 重现问题
   - `orionclaw browser trace stop`（打印 `TRACE:<path>`）
...
- `browser act kind=evaluate` / `orionclaw browser evaluate` 和 `wait --fn` 在页面上下文中执行任意 JavaScript。提示注入可能会操纵它。如果不需要，请使用 `browser.evaluateEnabled=false` 禁用它。
