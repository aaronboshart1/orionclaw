...
- `commands.config`（默认 `false`）启用 `/config`（读写 `orionclaw.json`）。
...
- 要获取完整的提供商使用量分解，使用 `orionclaw status --usage`。
- `/allowlist add|remove` 需要 `commands.config=true` 并遵循渠道 `configWrites`。
- `/usage` 控制每响应使用量页脚；`/usage cost` 从 OrionClaw 会话日志打印本地成本摘要。
...
- 覆盖立即应用于新的配置读取，但**不会**写入 `orionclaw.json`。
- 使用 `/debug reset` 清除所有覆盖并返回到磁盘上的配置。

## 配置更新

`/config` 写入你的磁盘配置（`orionclaw.json`）。仅所有者。默认禁用；使用 `commands.config: true` 启用。
