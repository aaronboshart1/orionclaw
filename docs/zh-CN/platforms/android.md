...
- 在 Gateway 网关主机的 `~/.orionclaw/orionclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上创建 `~/.orionclaw/workspace/canvas/index.html`。

2. 将节点导航到它（局域网）：

```bash
orionclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
