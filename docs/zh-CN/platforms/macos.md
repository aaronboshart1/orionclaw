...
summary: OrionClaw macOS 配套应用（菜单栏 + Gateway 网关代理）
title: macOS 应用
x-i18n:
...
# OrionClaw macOS 配套应用（菜单栏 + Gateway 网关代理）

macOS 应用是 OrionClaw 的**菜单栏配套应用**。它拥有权限，在本地管理/附加到 Gateway 网关（launchd 或手动），并作为节点向智能体暴露 macOS 功能。
...
- **本地**（默认）：如果存在运行中的本地 Gateway 网关，应用附加到它；否则通过 `orionclaw gateway install` 启用 launchd 服务。
...
如果 LaunchAgent 未安装，从应用中启用它或运行 `orionclaw gateway install`。
...
提示：与 `orionclaw gateway discover --json` 比较，查看 macOS 应用的发现管道（NWBrowser + tailnet DNS-SD 回退）是否与 Node CLI 基于 `dns-sd` 的发现不同。
