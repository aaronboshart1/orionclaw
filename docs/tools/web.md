...
OrionClaw ships two lightweight web tools:
...
If no `provider` is explicitly set, OrionClaw auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `orionclaw configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `orionclaw configure --section web`. It stores the key in
`~/.orionclaw/orionclaw.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.orionclaw/.env` (or your
...
environment. For a gateway install, put it in `~/.orionclaw/.env`.

If no base URL is set, OrionClaw chooses a default based on the API key source:
...
For a gateway install, put it in `~/.orionclaw/.env`.
