...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let OrionClaw run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask OrionClaw in plain language to "run this in Codex" or "start Claude Code in a thread", OrionClaw should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want OrionClaw-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | OrionClaw native sub-agent runtime  |
...
- OrionClaw binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, OrionClaw returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, OrionClaw uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, OrionClaw may default to persistent behavior per runtime path
...
If no target resolves, OrionClaw returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, OrionClaw returns a clear unsupported-control error.
...
When OrionClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal OrionClaw `agentId` path).
...
```bash
openclaw plugins install @orionclaw/acpx
orionclaw config set plugins.entries.acpx.enabled true
```
...
By default, `@orionclaw/acpx` uses the plugin-local pinned binary:
...
- Relative paths resolve from OrionClaw workspace directory.
...
- OrionClaw startup remains non-blocking while the backend health check runs.
...
```bash
orionclaw config set plugins.entries.acpx.config.permissionMode approve-all
orionclaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** OrionClaw currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
