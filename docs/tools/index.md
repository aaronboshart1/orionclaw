---
summary: "Agent tool surface for OrionClaw (browser, canvas, nodes, message, cron) replacing legacy `openclaw-*` skills"
read_when:
...
OrionClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `orionclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OrionClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openclaw`: all built-in OrionClaw tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`orionclaw node run`).
...
OrionClaw tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `orionclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OrionClaw-managed browser.
...
- Quick smoke: `orionclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `orionclaw gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, OrionClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OrionClaw clamps `tools.sessions.visibility` to `tree`.
