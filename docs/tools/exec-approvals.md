...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.orionclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.orionclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `orionclaw approvals allowlist ...`).
- `orionclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `orionclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.orionclaw/exec-approvals.json` directly.

CLI: `orionclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
