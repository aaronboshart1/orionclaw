...
read_when:
  - You need detailed behavior for orionclaw onboard
...
This page is the full reference for `orionclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.orionclaw/orionclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `orionclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `orionclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `orionclaw health`.
    - `orionclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.orionclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.orionclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.orionclaw/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`orionclaw onboard`](/cli/onboard)
