...
- With Anthropic API-key auth profiles, OrionClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, OrionClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
OrionClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.orionclaw/logs/cache-trace.jsonl" # optional
