...
OrionClaw features that can generate provider usage or paid API calls.
...
- `orionclaw status --usage` and `orionclaw channels list` show provider **usage windows**
...
OrionClaw can pick up credentials from:
...
- `orionclaw status --usage`
- `orionclaw models status --json`
...
`orionclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
