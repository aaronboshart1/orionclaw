...
OrionClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OrionClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- OrionClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
