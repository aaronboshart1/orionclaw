---
summary: "How OrionClaw builds prompt context and reports token usage + costs"
read_when:
...
OrionClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OrionClaw assembles its own system prompt on every run. It includes:
...
For images, OrionClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OrionClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `orionclaw status --usage` and `orionclaw channels list` show
...
`cacheWrite`. If pricing is missing, OrionClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OrionClaw can
...
Anthropic's 1M context window is currently beta-gated. OrionClaw can inject the
...
OrionClaw skips the `context-1m-*` beta header because Anthropic currently
