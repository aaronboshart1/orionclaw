...
- `pnpm test` on Node 24+: OrionClaw auto-disables Vitest `vmForks` and uses `forks` to avoid `ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`. You can force behavior with `OPENCLAW_TEST_VM_FORKS=0|1`.
...
Script: [`scripts/bench-model.ts`](https://github.com/aaronboshart1/orionclaw/blob/main/scripts/bench-model.ts)
...
Script: [`scripts/bench-cli-startup.ts`](https://github.com/aaronboshart1/orionclaw/blob/main/scripts/bench-cli-startup.ts)
...
This script drives the interactive wizard via a pseudo-tty, verifies config/workspace/session files, then starts the gateway and runs `orionclaw health`.
