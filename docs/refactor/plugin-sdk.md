...
Accessed via `OrionClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OrionClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OrionClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OrionClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OrionClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OrionClawConfig): string;
  };
};
```
...
- Add `api.runtime` to `OrionClawPluginApi` with the surface above.
