...
summary: "Architecture of OrionClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in OrionClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how OrionClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

OrionClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, OrionClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in OrionClaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createOrionClawCodingTools()
...
├── openclaw-tools.ts              # OrionClaw-specific tools
...
Image injection is prompt-local: OrionClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: OrionClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **OrionClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures OrionClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, OrionClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
OrionClaw wraps this with `guardSessionManager()` for tool result safety.
...
OrionClaw maintains an auth profile store with multiple API keys per provider:
...
OrionClaw loads custom pi extensions for specialized behavior:
...
OrionClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | OrionClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom OrionClaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.orionclaw/agents/<agentId>/sessions/` (or `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
