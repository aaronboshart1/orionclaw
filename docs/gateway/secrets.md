...
OrionClaw supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, OrionClaw performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). OrionClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/OrionClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
...
When reload-time activation fails after a healthy state, OrionClaw enters degraded secrets state.
...
- plaintext values at rest (`orionclaw.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `orionclaw.json`
...
- `orionclaw secrets configure --providers-only`
- `orionclaw secrets configure --skip-provider-setup`
...
OrionClaw intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, OrionClaw runtime no longer depends on plaintext `auth.json`.
