...
OrionClaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. OrionClaw requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. OrionClaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
OrionClaw only treats a request as Serve when it arrives from loopback with
...
- Set `gateway.tailscale.resetOnExit` if you want OrionClaw to undo `tailscale serve`
