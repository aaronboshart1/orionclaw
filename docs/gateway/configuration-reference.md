...
description: "Complete field-by-field reference for ~/.orionclaw/orionclaw.json"
summary: "Complete reference for every OrionClaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.orionclaw/orionclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OrionClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `orionclaw doctor` into `whatsapp/default`.
...
- OrionClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `orionclaw plugins install @orionclaw/mattermost`.
...
OrionClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `orionclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, OrionClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `orionclaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@orionclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `orionclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OrionClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OrionClaw assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.orionclaw/sandboxes`
...
noVNC observer access uses VNC auth by default and OrionClaw emits a short-lived token URL that serves a local bootstrap page; noVNC password is passed via URL fragment (instead of URL query).
    ...
    list: [
      {
        ...
        agentDir: "~/.orionclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@orionclaw"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, OrionClaw starts a fresh thread session instead of inheriting parent transcript history.
...
OrionClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.orionclaw/agents/<agentId>/agent/models.json`.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `orionclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `orionclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `orionclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `orionclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `orionclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `orionclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.orionclaw/extensions`, `<workspace>/.openclaw/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `orionclaw plugins update`.
...
Convenience flags: `--dev` (uses `~/.orionclaw-dev` + port `19001`), `--profile <name>` (uses `~/.orionclaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.orionclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Writes a unicast DNS-SD zone under `~/.orionclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `orionclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.orionclaw/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.orionclaw/credentials/oauth.json`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `orionclaw doctor --fix` can strip unknown keys).
