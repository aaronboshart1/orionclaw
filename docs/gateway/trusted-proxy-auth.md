...
read_when:
  - Running OrionClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OrionClaw
...
- You run OrionClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OrionClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OrionClaw extracts the user identity from the configured header
...
- OrionClaw can stay on loopback HTTP behind the proxy.
...
If OrionClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
routes:
  - from: https://openclaw.example.com
    to: http://orionclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`orionclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OrionClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `orionclaw security audit` and review findings
