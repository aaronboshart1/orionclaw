...
function candidateBinDirs(opts: EnsureOpenClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `orionclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "orionclaw");
  ...
  if (allowProjectLocalBin) {
    ...
    if (isExecutable(path.join(localBinDir, "orionclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `orionclaw` CLI can run
