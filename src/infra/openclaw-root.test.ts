...
describe("resolveOpenClawPackageRoot", () => {
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "orionclaw");
    const pkgRoot = path.join(project, "node_modules", "orionclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "orionclaw" }));
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "orionclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "openclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "orionclaw" }));
