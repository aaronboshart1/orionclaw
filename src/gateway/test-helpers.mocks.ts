...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `orionclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OPENCLAW_CONFIG_PATH = path.join(root, "orionclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "orionclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "orionclaw-gateway-test"),
        ...fileDefaults,
