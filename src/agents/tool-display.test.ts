  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.orionclaw/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.orionclaw/workspace",
        },
      }),
    );

    expect(detail).toContain("check git status -> show first 3 lines");
    expect(detail).toContain(".orionclaw/workspace)");
  });
