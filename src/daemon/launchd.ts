    ...
    if (isUnsupportedGuiDomain(detail)) {
      throw new Error(
        [
          ...
          "Fix: sign in to the macOS desktop as the target user and rerun `orionclaw gateway install --force`.",
          "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.orionclaw.local/gateway",
        ].join("\n"),
      );
    }
    ...
    if (isUnsupportedGuiDomain(detail)) {
      throw new Error(
        [
          ...
          "This usually means you are running from SSH/headless context or as the wrong user (including sudo).",
          "Fix: sign in to the macOS desktop as the target user and rerun `orionclaw gateway restart`.",
          "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.orionclaw.local/gateway",
        ].join("\n"),
      );
    }
