    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!linked) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("orionclaw channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (running && !connected) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("orionclaw doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
      }
    },
  });
