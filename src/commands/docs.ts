...
const SEARCH_TOOL = "https://docs.orionclaw.local/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.orionclaw.local");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('orionclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.orionclaw.local/");
      runtime.log(`Search: ${formatCliCommand('orionclaw docs "your query"')}`);
    }
    return;
  }
