...
export async function statusCommand(
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`orionclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("orionclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("orionclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("orionclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("orionclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.orionclaw.local/faq");
  runtime.log("Troubleshooting: https://docs.orionclaw.local/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("orionclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("orionclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("orionclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("orionclaw gateway probe")}`);
  }
}
