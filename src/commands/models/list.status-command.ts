...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("orionclaw models auth setup-token")}\` or \`${formatCliCommand("orionclaw configure")}\`.`
          : `Run \`${formatCliCommand("orionclaw configure")}\` or set an API key env var.`;
