 ...
 * Runs as part of `orionclaw doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("orionclaw config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("orionclaw memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("orionclaw configure --section model")}`,
        `- To disable: ${formatCliCommand("orionclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("orionclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
