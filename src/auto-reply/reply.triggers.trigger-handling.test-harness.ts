...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPENCLAW_STATE_DIR = join(home, ".orionclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "orionclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".orionclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
