...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "orionclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "orionclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[orionclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[orionclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[orionclaw] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[orionclaw] ${parsed.error}`);
      process.exit(2);
    }
