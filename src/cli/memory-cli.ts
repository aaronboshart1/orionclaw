    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["orionclaw memory status", "Show index and provider status."],
          ["orionclaw memory index --force", "Force a full reindex."],
          ['orionclaw memory search --query "deployment notes"', "Search indexed memory entries."],
          ["orionclaw memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.orionclaw.local/cli/memory")}\n`,
    );
