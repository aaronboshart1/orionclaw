...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "orionclaw", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "orionclaw", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "orionclaw", "status"]);
  });
