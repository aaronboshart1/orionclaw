...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("orionclaw");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "orionclaw", "status", "--debug"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "orionclaw", "message", "send"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "orionclaw", "--version"],
    });
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "orionclaw", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "orionclaw", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "orionclaw", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
