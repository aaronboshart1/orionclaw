...
${formatHelpExamples([
  ['orionclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['orionclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'orionclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'orionclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['orionclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'orionclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.orionclaw.local/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.orionclaw.local/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'orionclaw agents set-identity --agent main --name "OpenClaw" --emoji "🦞"',
    "Set name + emoji.",
  ],
  ["orionclaw agents set-identity --agent main --avatar avatars/openclaw.png", "Set avatar path."],
  [
    "orionclaw agents set-identity --workspace ~/.orionclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "orionclaw agents set-identity --identity-file ~/.orionclaw/workspace/IDENTITY.md --agent main",
