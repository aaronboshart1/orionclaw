...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "orionclaw", "doctor"]);
  ...
  it("does not narrow to the primary command when help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "orionclaw", "doctor", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "orionclaw", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "orionclaw", "status"], async () => {
      await program.parseAsync(["node", "orionclaw", "status"]);
    });
