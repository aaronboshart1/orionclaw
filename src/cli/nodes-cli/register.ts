    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["orionclaw nodes status", "List known nodes with live status."],
          ["orionclaw nodes pairing pending", "Show pending node pairing requests."],
          ['orionclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["orionclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.orionclaw.local/cli/nodes")}\n`,
    );
