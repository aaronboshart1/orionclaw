...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("orionclaw doctor")}" (or "${formatCliCommand("orionclaw doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("orionclaw gateway install --force")}\` from the same --profile / OPENCLAW_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("orionclaw gateway install")}`),
    );
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("orionclaw status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.orionclaw.local/troubleshooting`);
}
