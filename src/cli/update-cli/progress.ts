...
const STEP_LABELS: Record<string, string> = {
  ...
  "orionclaw doctor entry": "Checking doctor entrypoint",
  "orionclaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g orionclaw@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g orionclaw@latest --omit=optional");
  }
