...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["orionclaw security audit", "Run a local security audit."],
          ["orionclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["orionclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["orionclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.orionclaw.local/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("orionclaw security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("orionclaw security audit --fix")}`));
        if (!fixResult) {
