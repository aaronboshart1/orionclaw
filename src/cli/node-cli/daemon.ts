...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("orionclaw node install"),
    formatCliCommand("orionclaw node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("orionclaw node install --force")}`);
    }
    return;
  }
