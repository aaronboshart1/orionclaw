...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.orionclaw.local/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        lines.push(
          ...
          `  ${theme.command("orionclaw devices list")}`,
          `  ${theme.command("orionclaw devices approve <requestId>")}`,
        );
