...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["orionclaw update", "Update a source checkout (git)"],
        ["orionclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["orionclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["orionclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["orionclaw update --dry-run", "Preview actions without changing anything"],
        ["orionclaw update --no-restart", "Update without restarting the service"],
        ["orionclaw update --json", "Output result as JSON"],
        ["orionclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["orionclaw update wizard", "Interactive update wizard"],
        ["orionclaw --update", "Shorthand for orionclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run orionclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.orionclaw.local/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["orionclaw update status", "Show channel + version status."],
          ["orionclaw update status --json", "JSON output."],
          ["orionclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.orionclaw.local/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
